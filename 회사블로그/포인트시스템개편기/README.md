...
Beanstalk Worker에 대해 간단하게 소개드리면 Beanstalk 내부에 SQS 메세지 리스너 데몬을 두고 해당 데모인 어플리케이션으로 대신 HTTP API 요청을 보내는 환경을 얘기합니다.
...
그래서 모든 포인트 데이터는 Insert만 존재하는 도메인 모델로 구현하였습니다.  
기본적으로 적립이나 조회에 대해서는 크게 어려움이 없었습니다.  
하지만 몇몇 경우에선 문제가 되었습니다.

### 2-1. 포인트 사용 & 차감

예를 들어 1000원의 포인트를 유저가 사용했다면 단순하게 생각하면 -1000원이란 포인트 데이터를 한줄 추가하면 됩니다.  
하지만, 그럴수가 없었습니다.  
바로 **유효기간만료** 때문입니다.  
...
만약 포인트가 사용되는 이벤트가 즉시 처리 되지 않으면, 사용자는 어플리케이션이 메세지를 수신 받기전에 다시 같은 포인트를 사용할 수 있게됩니다.  
그래서 포인트 사용 이벤트는 즉시 처리되고 한 트랜잭션에서 끝내기 위해 API역할을 하는 point-edge 모듈에서 즉시 처리하도록 구현하였습니다.  
...
아직 테스트에 대해서 높은 단계에 있지 못했기 때문에 **단위 테스트가 통과해도 통합테스트에서는 실패하는 경우**가 종종 있었습니다.  
단위 테스트는 필수, 통합 테스트는 선택으로 해도 좋지 않을까 싶었습니다.  
하지만 **과한 Mocking**과 **잘못된 단위 설정** 등이 몇번 발생하니 **통합 테스트도 필수로 짜서 실제 전체 프로세스를 수행해도 문제가 없는 것을 확인하는 것**까지로 결정했습니다.  
