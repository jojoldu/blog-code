...
* SQS Worker에서 메세지를 초당 얼만큼 처리하는지에 대한 검증
* Redis 조회 처리 검증
...
이때 QA 분들께 정말 죄송했던 것이, 구 API와 신 API를 각각 테스트해야한다는 것을 미리 얘기해드리지 못해 안그래도 부족했던 QA 일정을 더 촉박하게 만들었던 점입니다.  
...
Redis Worker로 메세지를 발행하는 것은 **트랜잭션이 반영되지 전**입니다.  
...
이렇게 할 경우 오픈 시간 동안 (약 30분) 포인트 시스템에는 전혀 데이터가 추가되지 않기 때문에 깔끔하게 전환 할 수 있습니다.  
...
이 검증까지 끝나고 나서는 완전히 코드를 제거하였습니다.  

포인트 시스템이 완전히 개편되었습니다!  
