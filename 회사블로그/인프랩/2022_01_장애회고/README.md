...
본격 트래픽이 유입되니 1월 3일 **오후 11:30분부터 1월 4일 새벽 01:30까지** 2시간동안 **Write DB의 CPU가 90% 이상**이 되어 전체 서비스가 사용이 불가능했습니다.  
  
![4](./images/4.png)

이번 장애의 경우 Writer DB의 부하로 인해 발생했습니다.  
  
이땐 2개의 문제가 겹치게 된 경우였습니다.  

* Redis의 부하를 낮추기 위해 DB로 분산시킨 캐시용 데이터
* Writer DB에서 호출되던 조회용 쿼리
...
그렇게 적재되던 고용량의 캐시 Row는 (단일건) 결국 캐시 초기화 / 캐시 재생성등의 행위가 발생할때마다 Writer DB에게 큰 부하를 주게 됩니다.  
...
아직 **Writer DB에서 처리하는 조회용 쿼리**들의 호출수도 급격히 높아져 Writer DB의 부하를 더욱 심화시켰습니다. 

> 인프런의 경우 **Aurora DB 도입을 작년 여름**에 하여 Writer / Reader DB 쿼리 분산이 일부 쿼리에만 적용된 상태였습니다.  

서비스를 안정화시키는 것이 우선이기에, 당장 조치하기 어려운 캐시 데이터 해체보다는 아래 2가지 작업을 우선적으로 진행합니다. 

* 캐시 초기화 & 재생성 주기 변경
* Writer DB에 부하를 준 조회용 쿼리들만 먼저 Reader DB로 전환
...
이 쿼리들은 당시 모니터링상으로 크게 2가지 문제가 있다고 판단했습니다
...
슬로우 쿼리들의 대부분은 **테이블의 컬럼을 변조**하거나 부정조건 (`!=`, `NOT IN` ) 등 인덱스를 사용할 수 없는 형태의 쿼리들로 되어 있었습니다.

```sql
lower("컬럼명") = lower(쿼리파라미터)
```

그러다보니 테이블 스캔 쿼리가 동시에 발생하면서 여러 병목의 원인이 되었습니다.  
...
이 2가지 큰 문제들을 바로 개선을 진행했습니다.

* 조회 쿼리 개선
  * 인덱스를 추가하거나 쿼리를 변경해서 인덱스를 사용하도록 개선
  * 개선이 안되는 쿼리라면 차라리 기능 OFF
* `select *` 로 조회되던 컬럼에서 고용량인 강의 소개 컬럼은 모두 제거
  * 실제 해당 기능에서 사용되지 않았기 때문에

원인을 파악하고 문제가 되는 쿼리들을 정리하는데는 모니터링과 슬로우쿼리 알람을 통해 30분도 안되는 짧은 시간이 소요된 반면, 실제 해결까지는 3시간이 소요되었는데요.  
...
쿼리 내용을 전부 문자열 함수/ Hook / 커스텀 이벤트를 사용해서 구성하다보니 **해당 쿼리의 실제 코드를 찾는데 많은 시간을 소모**하게 됩니다.  
...
위 조치사항을 취하고, 서비스가 안정화 되는 것을 확인후 퇴근을 할 수 있었습니다.

## 1월 7일 장애 

1월 4일이 지나고 1월 5일 ~ 6일 양일간에는 더이상 장애가 발생하지 않았습니다.  
그래서 이젠 이벤트를 통한 병목 현상은 모두 해소되었다고 안심하게 되는데요.  
  
![facebook](./images/facebook.png)

(저와 함께 설레발치셨던 대표님 ㅠ)  
  
1월 7일 저녁 9시 30분부터 다시 트래픽이 늘면서 서비스의 지연현상이 시작 됩니다.  
