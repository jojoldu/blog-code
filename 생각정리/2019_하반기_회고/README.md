...
[상반기 회고](https://jojoldu.tistory.com/436) 에도 작성했지만, 7월 1일자로 팀이 분리되어 팀내 역할이 변경 되었다.  
...
야구 선수 중에 [오타니 쇼헤이](https://namu.wiki/w/%EC%98%A4%ED%83%80%EB%8B%88%20%EC%87%BC%ED%97%A4%EC%9D%B4) 라는 선수가 있는데 (만다라트 계획표로 유명한 그 선수) 이 선수가 투수와 타자 모두 에이스 역할을 해서 별명이 **이도류** 라고 한다.  
  
팀에서 함께 했던 호돌맨의 인상이 딱 이 선수 같은 느낌이였다.  
백엔드 애플리케이션도 잘하는데, **인프라도 너무 잘하는 것이였다**.  
  
당시에 50~60대에 달하는 결제/정산 IDC 인프라를 모두 호돌맨님이 관리중이였다.  
...
> 그래서 실제로 이 회사 출신 개발자분이 한분 더 계시는데, 그 분 역시 인프라 레벨이 장난아니더라.
> 집에 직접 몇백만원짜리 서버를 두고 관리하고 렉 꽂아서 놀이를 한다고 들었다.

> 후일담이지만, 호돌맨님 퇴사하실때 회사 SRE 팀에서도 팀 이동 제안을 했었다.



퇴사 얘기야 그 전부터 이야기하시긴 했는데, 실제로 회사에 공식적으로 이야기를 10월에 하셨다.  
...
3년간 개발에 대해 같이 이야기를 나누던 분이 떠나니 내년엔 어떤 재미를 찾아야 하나 고민이 된다.
