...
하지만 기존 인프런 레거시에서는 그대로 JS/Express/FxDom (like jQuery) 을 사용하고 있었다.  
...
"왜 지금와서 IoC/DI, AOP 프레임워크가 인기가 있는거지..?"  
"기능이 훨씬 부족한 상태인데?"
...
사용해보면서 느낀점은... "**TS때문에 쓴다**" 였다.  
프레임워크 완성도는 만족하기 힘들었다.  

보통 NestJS가 Spring을 대체한다고 하던데, 실제로 써보면 Spring의 여러 종류 중 정확히 **Spring Web(MVC)에 대한 부분만 대체**가능했다.  

물론 대부분의 서비스는 **웹 API로만 대부분의 기능을 구현하니** Full NestJS 스택으로 구현해도 문제는 없다.  
서비스가 커지고나면 API 외 다른 방법들을 고민해도 된다고 생각한다.  
  
다만, Spring Data, Spring Security, Spring Batch, Spring Cloud 등 엔터프라이즈 애플리케이션에 필요한 여러 다른 부분은 Nest가 대체해줄수는 없다.  
애초에 이쪽 추상화/구현체가 없다.  
그러다보니 좋은 해결책 보다는 아쉬운 해결책을 자주 선택할 수 밖에 없었다.   
  
몇번 그런 결정들이 생기다보니 NestJS가 가진 장점 보다는 **TS라는 좋은 언어를 프레임워크에 적용할 수 있는 유일한 해결책** 이라서 지금의 인기를 가진게 아닐까? 하는 생각이 들었다.  
...
통합 테스트 환경을 구성할때도 역시 어노케이션 (`@SpringBootTest`) 로 한번에 해소되지 않고, `TestingModule` 등록이 필요하다.  
...
NoSQL + 메세지큐는 너무 흔한 조합이지만, 우리팀은 이제야 이걸 도입했다.  
(특히 메세지큐가 2022년에 도입된게 아쉽다. 흔하디 흔한 조합인데)  
...
MongoDB는 Node.js와 궁합이 잘 맞는다.  
실제로 DynamoDB 보다는 Node.js 개발자였던 우리팀에는 MongoDB가 훨씬 좋았다.  
하지만 Node 가 아닌 Kotlin & MongoDB를 선택했다.  
...
* AWS와 같은 Cloud를 사용할때 Spring Cloud가 지원하는 풍부하고 강력한 기능들이 있다.
  * Node.js에서는 Spring Cloud와 같은 기능이 풍부하면서도 테스트로 기능이 검증된 **추상화 된 라이브러리가 없다**.
...
* BCI (`bytecode instrumentation`) 등의 지원이 없어 APM 모니터링에 불편함이 많다.
  * Monkey patching 이 지원되지만 
  * Datadog, NewRelic 
  
스타트업은
기술 부채 관리의 예술
필요할 때 얼마나 갚기 쉬운 형태로
기술 부채를 만들어서 비즈니스를 성장시킬 것인가
최소한의 도구로 다양한 문제들을
일정 수준 이상으로 해결 가능

* RDBMS만 사용하고 있고
* RDBMS의 Full Text Search로 검색을 하면서 
* 비정형 데이터베이스 도입이 예정되어있는 

그런 소규모 스타트업은 고려해볼만한 옵션

> MongoDB Korea 팀의 적극적인 Support가 소규모 스타트업 팀에게는 꿀이다.
...
원티드의 창업자 3분과 쭈, 나 이렇게 5명이서 식사시간을 가진적이 있다.  
그러면서
