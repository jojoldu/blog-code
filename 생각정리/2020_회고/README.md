...
소상공인을 위한 **우대수수료**를 가장 먼저 적용한 것도 우리 회사인데 (위 표를 보면 알겠지만 3%가 안된다), 진짜 많은 부분에서 업주님들을 위한 서비스를 만들고도 네이버 카페의 댓글을 볼때면 안타까웠다.

> 서비스라는 것에 있어서 이렇게 감성 젖는 이야기는 **찌질하다는 이야기를 하도 들어서** 요새는 이런 이야기 잘 하지 않는다.    
...
한번만 하고 지원이 끝나겠거니 했는데, **1년내내 정기적으로 지원**하게 되었다.   
단발성이란 생각에 수동 운영할걸 감안하고 빠르게 만들었다가 주기적으로 사용하게 되다보니 조금은 아쉬움이 있었다.  
하지만 어차피 시간 관계상 더 고민하고 만들 수 있는 상황은 아니였다보니 그 당시 최선의 선택이였다고 생각한다.   

> 물론 이런 이야기로 회사르 자꾸 쉴드치는 모습에 "인생 최대 업적이 배민 입사" 라던가 "회사랑 본인을 일치화시키네" 같은 이야기를 할 수 있는데, 나는 이미 퇴사한 사람이라서 **인생 최대 업적이 배민 입사인 사람은 아니다**.
...
해외원서는 사내 복지중 하나인 **사파리북스 계정 지원**이 있어서 온라인으로 편하게 볼 수 있었다.  
  
특히나 그동안 MySQL에 대해서는 시스템 운영 외에 쿼리 튜닝에 대해서는 제대로 보지 못했는데, 이걸 해소하게 되어서 좋았다.  
...
기술 블로그의 경우엔 내 개인 블로그에 쓰고 싶었지만, 우리 팀을 홍보하기 위해서는 사내 기술 블로그에 올리는게 맞다고 생각되었다.  
  
[다른 글](https://jojoldu.tistory.com/562)에서도 언급했지만, 내가 생각하는 숙제중 하나가 팀 브랜딩이였기 때문에 외부 홍보에 있어서는 적극적으로 나섰다.

![notion](./images/notion.png)

([팀 채용 노션](https://woowahan.oopy.io/60a06399-3f95-4fec-a436-000ad6baff40))  
  
결과적으로는 이 notion 포맷을 다른팀에서도 많이 차용해주셔서 하길 잘했다는 생각을 했다.  
...
다행히 그런 활동이 도움이 되었는지 다음 해 신입 개발자분들이 가장 많이 지원한 팀이 되기도 했고, 경력 공채에서도 가장 많은 지원을 받은 팀이 될 수 있었다.  
  
소기의 목적은 달성한것 같아 내심 뿌듯했다.
...
2019년 12월은 아무래도 EO 영상 덕분에 말도안되는 수치를 달성했기 때문에 정확한 비교로 보기는 어렵다.  
(2019년 11월은 MAU 4.4만, 월PV 12만 이기 때문이다.)  
  
막상 2020년엔 어떤 컨텐츠가 노출되었느냐에 따라 차이가 심했는데, 6월의 경우엔 MAU 6.3만, 월PV 18.4만이였다 (12월에 비해 훨씬 더 높다.)

![ga2](./images/ga2.png)

어찌됐든 2019년의 총 사용자수가 39.4만, 총 PV가 161만이였던 것에 비하면 2020년 한해는 전체적으로 20%의 성장을 했다.
...
역시 이런 에세이류? 주니어 개발자분들이 좋아할 일반 컨텐츠가 인기 있는건가? 라는 생각이 들었다.  
그래서 2021년에는 **에세이보다 인기 있는 기술 컨텐츠**를 만들어보는걸 목표로 삼아야겠다.
...
> 블로그 400만 회고는 별도의 글로 정리할 예정이였는데, 벌써 500만을 돌파해서 그냥 500만 회고를 지금부터 준비하는게 낫겠다.
...
사내 첫 기술 컨퍼런스 우아콘에서 [수십억건에서 Querydsl 활용하기](https://www.youtube.com/watch?v=zMAX7g6rO_Y) 주제로 발표를 했다.

![woowacon](./images/woowacon.png)

2020년에는 꼭 회사 이름을 걸고 기술 컨퍼런스를 하자는 이야기가 있었는데, 거의 10월이 되어서야 본격적으로 시동을 걸게되었다.  
  
그러다보니 12월에 컨퍼런스 오픈을 하려면 발표 준비를 한달만에 끝내야만 했다.  
...
여기서 후자를 선택한 이유는 컨퍼런스 전체 주제들과의 조합을 맞추기 위해서였다.  
  
우아콘을 보신분들은 아시겠지만, 모든 주제가 **코드가 없고, 아키텍처와 관련된 내용**들이였다.  
그러다보니 기술 컨퍼런스인데 실무 코드가 전혀 없는 상황인것이였다.  
  
그래서 그 중 하나는 실무코드가 있어서 주제가 골고루 나눠지는게 필요하다고 다른 발표자분들이 의견을 주셨고 나도 그 의견에 동의해서 Querydsl로 주제를 선택하게 되었다.  


원테이크로 찍었어야 했다.  
개인당 1시간정도의 촬영 시간이 있었고, 이에 대해서 


### 4-5. 코디미

호돌맨과 코디미 사이드 프로젝트를 시작했다가 **접었다**.  

![coordime](./images/coordime.png)

호돌맨과는 이런 사이드 프로젝트에 관한 이야기를 자주 하는데, 지금의 [인프런 멘토링](https://www.inflearn.com/mentors)과 같은 서비스가 재밌을것 같아서 시작했다.  
...
500ml 텀블러 기준으로 4~6컵을 매일 마시고 있으니 어느정도 계획은 성공한것 같다.
