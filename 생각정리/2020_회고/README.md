...
"그냥 정산시스템이 계속 진행하면되지않나?" 라고 하기엔 너무나 어려웠던 것이,
...
왜 그런 큰 프로젝트를 A/B 테스트를 거치지 않았냐부터 시작해서 별별 말들을 주변에서 많이 들었는데, **단순히 UI/기능의 변경이 아닌** 고객/업주/라이더 분들에게 금전적인 영향을 패치가 그냥 A/B 테스트로 되는게 아니다.  
(막말로 A/B 테스트로 누구는 같은 가격에 물건을 팔아도 덜 수수료 주고, 적게 주는게 말이 안되지 않을까?)  
  
할말은 많은데, (어찌됐든 퍼블릭한 공간이니깐 넘어가서) 그렇게 큰 프로젝트가 오픈되고 롤백 되는 과정을 경험하면서 서비스 기업의 어려움을 많이 느끼게 되었다.  
  
O2O + 다양한 이해관계가 묶인 서비스는 정말 어렵다는 생각을 했다.  
...
그래서 서비스마다 이 중에서 누구를 만족시킬지 결정하고, **나머지 분들의 혜택을 빼와서** 그쪽의 헤택을 몰아주게 된다.  
...
그래서 오로지 고객분들을 위해 모든걸 한다는 것은 **업주님들과 라이더분들에 드리는 혜택에서 가져올 수 밖에 없다**.  
...
90%이상의 주문이 발생하는 배달의 민족에서 주문 건당 발생하는 수수료가 없으니 무언갈 시도하기가 어렵다.  
  
> 소상공인을 위한 우대수수료를 가장 먼저 적용한 것도 우리 회사인데 (위 표를 보면 알겠지만 3%가 안된다), 진짜 많은 부분에서 업주님들을 위한 서비스를 만드는 것에 비해 가끔 네이버 카페와 댓글을 볼때마다 안타깝다.
...
작년 2분기부터는 코로나 지원으로 인해 많은 리소스가 투입되었다.  
...
이런 혜택은 참 많았다.  
작년 한해는 포장 주문일 경우 결제 수수료를 **우리가 대신 PG사에 납부했다**.  
즉, **포장주문은 하면 할수록 결제수수료를 우리가 대신 내는 모델**인데, 
...
> ps1) 뭐 어차피 서비스라는 것에 있어서 이렇게 감성 젖는 이야기는 **찌질하다는 이야기를 하도 들어서** 요새는 별로 신경쓰지 않는다.  
> 결국은 서비스 품질이 제일 중요하지 않냐는 이야기로 결론이 나기 때문이다.  
> 다만, 다른 서비스처럼 업주님들을 위한 여러 지원 혜택도 하지 않고 주문 수수료도 빵빵 하게 받으면서 욕 먹었으면 억울하지도 않았을텐데 하는 아쉬움은 있다.  
> 서비스 품질이 좋으면 업주님들과 라이더분들께는 혜택을 줄이는게 맞나?? 라는 생각은 아직 있는데, 참 사업은 어렵다.

> ps2) 이 글 보고 "인생 최대 업적이 배민 입사" 라던가 "회사랑 본인을 일치화시키네" 같은 이야기를 할 수 있는데, 일단 난 애사심이 엄청 높은 사람이 아니다.  
> 현 회사에 충성하는 사람이 아니라는 것은 이후에 다른 글이나 유튜브에서 이야기할 예정이다.
