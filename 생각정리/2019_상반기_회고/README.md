...
결국 매일매일 정산금이 나가기 때문에 한번 삐끗하면 큰일이기 때문이다.  
...
단지 옆에서 조언을 드리고, 샘플 코드를 작성해서 보여드리고, 설계 회의 참석하는 등등의 간접적으로 도움을 드렸다.  
  
좀 더 도움을 드리고자 진행중이시던 프로젝트 코드를 분석했다.  
워낙 프로젝트가 커서 전체 코드를 다 파악하진 못했다.  
다만 내가 파악한 선에서 내용들을 정리해서 해당 팀에 발표를 했다.
...
좀 더 적극적으로 코드도 작성하고 도와드리면 더 좋았을텐데.  
...
팀장님이 기획자이시다 보니 개발 파트의 주변 일들을 내가 하게 되었다.  

IDC 인프라 관리/이력서 검토/면접/개발 회의 등등 기타 작업들을  해야만 한다.  
...
그렇다고 완전히 손 놓은건 아니다.  
비동기 API와 같이 성능 개선이 필요한 작업들은 직접 진행하기도 한다.  
  
다만 다른 분들이 개발에 집중할 수 있게 기타 작업들 위주로 하고 있고, 최근엔 리눅스 쉘 스크립트 작업 비중이 높았다.  
  
같은 팀이 된 기획자분들 이야기를 조금 하고 싶다.  
