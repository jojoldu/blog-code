...
하지만, 모든 웹 애플리케이션은 **서비스가 성공을 하고나면 100% 새로운 기능을 요구받게 된다**.  
...
서로간의 행동을 실행시키기 위해서는 협력 객체에 대한 어느정도의 이해도가 필요한데,  
...
위 예제를 통해 더이상 클래스는 단순한 데이터를 나타내기 위함이 아닌 것을 알 수 있다.  
  
객체의 타입을 안다는 것은 그 객체가 어떻게 행동할지 예상할 수 있다는 것이다.  
즉, 이 객체가 **어떤 메세지에 반응하는지 예상할 수 있다**는 것이다.  
그래서 우리가 객체지향 디자인을 한다고하면, 이 객체가 어떤 행동을 할지 예상할 수 있도록 코드를 배치해야만 한다
...
그렇기 때문에 우리가 할 것은 애플리케이션이 어떤일이 벌어질지 예측하는것이 아니라, **어떤 애플리케이션이든 변하다는 사실을 인지하고, 지금은 그 변경이 무엇인지 예측할 수 없다는 것을 인정하는 것**뿐이다.  
