...
즉, 개발자의 문제를 떠나 서비스적인 문제로 확장될 수 있기 때문에 계층화는 필요한 작업이다.
...
* 여기서의 DTO는 **기능이 제한된** 도메인 객체
* 혹은 View 전용 등의 도메인과 무관하게 데이터 전달만을 위한 객체를 뜻한다
...
다만, 나 같은 경우 서비스와 기반 서비스로 나누자니 기존에 `@Service` 에 모든 Business 로직을 넣는 분들이 오해하실 때가 있어서 아래와 같이 좀 더 명확하게 (`@Service` 에 대해서 자연스럽게 넘어갈수 있도록 ) **도메인 / 서비스** 계층으로 부르는걸 선호한다.

* 도메인 계층
  * Rich Domain 모델을 기반의 **순수 도메인 문제 해결에 집중하는 계층**
...
의존하게 될 경우 다음과 같은 문제점들이 발생한다.

* 도메인 로직을 다른 애플리케이션 로직에서 재사용하기 어렵다
*
...
이렇게까지 하면서까지 계층화를 해야하는 이유 (서두에서도 간단하게 언급했지만)는 크게 3가지가 있다.
...
DataAccess 계층내에서 SQL에 대한 변경 사항이 Business 계층과 Presentation 계층 모두에 영향을 미치게 된다.  
즉, 계층간 결합도가 매우 높은 애플리케이션이 생성되는 것이다.  
...
그렇게 각 계층을 넘나드는 개발을 하더라도 충분히 격리된 계층화를 사용하게 되면 한번에 한 계층에만 집중할 수 있는 환경을 구축할 수 있다.  

이런 점을 마틴 파울러는 마치 [리팩토링](http://www.yes24.com/Product/Goods/89649360)의 두 가지 모자를 사용 하여 사고 모드를 전환하는 것과 유사하다라고 표현했는데, 굉장히 공감이 간다.
