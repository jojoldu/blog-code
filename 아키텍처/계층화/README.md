...
* Domain(Business or Service) 계층
  ...
  * 애플리케이션이 수행해야하는 **도메인**과 관련된 작업들을 담당한다.
    ...
    * 프레젠테이션 계층에서 받은 데이터의 유효성 (Validation) 검사
...
최근엔 (개인적인 경험으로) 아래와 같이 좀 더 풍부한 계층을 가진 형태를 사용하기도 한다.

![2](./images/2.png)

비지니스 로직을 담당하는 Domain 계층을 좀 더 세분화한다. 

* 도메인 계층
  * Rich Domain 모델을 기반으로한 순수 **도메인 문제 해결에 집중하는 계층**
  * 비지니스 로직만 담당하며, 외부의 특정 기술이나 구현 의존성을 최대한 피한다.
* 서비스 계층
...
DTO 계층이 없다면 대표적으로 다음과 같은 경우에 도메인 오브젝트가 손상될 확률이 높다.

* 프레젠테이션 영역에서 필요한 로직과 도메인 로직을 분리할 수 있다.
  * 단순히 화면 노출에 필요한 연산 (날짜 타입 직렬화 등)은 도메인 로직이 담당하는 것이 아니다.
  * 도메인 오브젝트를 
* 도메인 로직이 오염된다.
  * 도메인 오브젝트는 단순히 값 조작과 연산이 들어가는게 아니라 주요한 도메인 로직을 담고 있다.
  * 화면 노출 혹은 외부 API 제공에 도메인 오브젝트가 사용된다면 `public`, `private` 등의 scope 오염부터 시작해서 굉장히 많은 부분에서 도메인 로직이 오염된다.
* 통계나 지표 등의 데이터가 필요할 경우 여러 테이블의 컬럼값을 조합해서 만들어진다.
  * 이런 경우에는 적절한 도메인 오브젝트를 선택하기 보다는 DTO를 통해 단순히 값만 담은 형태로 제공한다.
