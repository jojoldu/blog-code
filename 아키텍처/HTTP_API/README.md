...
새로운 권한이 추가되고, 해당 권한은 기존의 API를 사용한다면 그 API의 URL이 적절한가?  
한 눈에 이 API를 사용해야 함이 알아볼 수 있는가?  

- 일반 유저 조회 API: GET `/users/${userId}/summary`
- 관리자 유저 조회 API: GET `/users//${userId}/detail`


두 방법 모두 상황에 따라 유용하게 사용될 수 있으며, 특정 프로젝트의 요구사항과 팀의 선호도에 따라 선택하면 된다. 그러나 일관성 유지가 중요하므로, 한 가지 방식을 선택했으면 프로젝트 전체에서 일관되게 사용하는 것이 좋다. 일반적으로, RESTful API에서는 URL 경로 방식을 더 많이 사용하지만, 쿼리 파라미터 방식도 특정 요구사항에 따라 매우 유용하게 사용될 수 있다.
