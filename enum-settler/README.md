...
상반기 저희팀 최대 과제인 정산 플랫폼 개편을 진행함에 있어 저에게 큰 힘이 되었던 것은 **Enum**이였습니다.  
이미 기존의 많은 블로그와 책에서 Enum의 정의와 기본적인 내용을 소개했기 때문에, 여기서 소개드릴 것은 "**프로젝트를 진행함에 있어 발생한 문제를 Enum을 통해 어떻게 해결했는가**" 입니다.  
어마어마한 비법이나 신기술을 담은 내용은 없지만, "얘는 이런 것도 몰랐구나" 라는 넓은 마음으로 봐주시면 될것 같습니다.
...
(이 글의 제목이 Enum 활용기가 아닌, **Java Enum 활용기**인것도 이 때문입니다.)  
  
특히 DB 중심의 개발에서 **객체 중심의 개발로 옮기는데 아주 좋은 도구**가 된다고 생각되어 이 글을 작성하게 되었습니다.  
...
* 불필요한 코드량이 많습니다.
  ...
  * 동일한 타입의 값이 추가되는데 너무 많은 반복성 코드가 발생하게 됩니다.
...
> JPA를 사용하시는 경우 위 처럼 ```@Enumerated```를 선언하시면 Enum 필드가 테이블에 저장시 숫자형인 1,2,3이 아닌, Enum의 name값 그대로가 저장됩니다.  
...
여기까지 진행 후 코드를 다시 보니 한가지 더 문제가 있었습니다.  
결제수단이 **문자열**인 것입니다.  
...
특히나 카테고리의 경우 **6개월에 1~2개가 추가될까말까**한 영역이기 때문에 굳이 테이블로 관리하는 것은 장점보다 단점이 더 많다고 생각하였습니다.  
...
이젠 이 과정이 좀 줄어들어 편해짐을 직접적으로 느끼고 있습니다.  
(역시 정적언어 짱!?)  
