...
이렇게 메소드로 분리하고 실제로 사용해보면,  
코드는 코드대로 조회하고 계산은 또다시 별도의 클래스&메소드를 통해 진행해야함을 알 수 있습니다.
...
더불어 역활과 책임이라는 관점으로 봤을때, A Code는 a 메소드를 수행해야 한다는 것에 책임이 있는 것은 A Code라고 생각하였습니다.  
...
물론 내부적으로는 인터페이스를 사용하니 완전하다고 할순 없습니다^^;)  
