...
* 리더가 가르쳐주는 것을 유독 빠르게 배우는 팀원이 있는 반면, 그렇지 못한 팀원도 반드시 있다.
  ...
  * 모든 선수에게는 각자의 학습곡선이 있다.
* 좋은 리더는 필요에 따라 여러 역할을 할 수 있어야 한다
  * 훈련, 시범조교
  * 카운슬러
  * 롤모델
  * 동기부여자
  ...
  * 인재 감식가
  * 심판
  * 처음 UCLA에 왔을때는 청소부 역할을 하기도 했다.
* 말 보다는 시범이 더 강력하다.
  * 리더라면 말 보다는 직접 행동으로 보여줘라.
  * 다른 사람에게 예의를 갖추는 것이든, 지각하지 않는 것이든, 자유투를 하는 것이든, 자제력이든 간에 모두 마찬가지로 **행동이 말보다 더 큰 목소리를 낸다**.
* 선수들에게 너무 많은 가이드를 전달하지마라.
  * 훈련 규칙, 개인 트레이닝 방법, 태도, 복장 규정 등 많은 안내서를 제공하면 선수들은 한번에 소화할 수 없다.
  * 그들이 쉽게 소화하고 이해하고 활용할 수 있을 정도의 크기로 정보를 잘게 나누어 먹이는게 효과적이다.
  * 