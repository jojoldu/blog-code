...
정적분석 도구를 SonarQube에서 젯브레인의 Qodana로 옮겼음.
SonarQube는 코드 라인 수(LOC) 기준 과금이고, 100만 라인 단위로 계약이 끊김. 우리 저장소는 0.5M에서 1M, 다시 2M으로 티어를 두 번 올렸음.
100만 라인을 쓸 때 연 $4,220이었는데 200만 라인으로 올리니 연 $9,757이 됨. 라인 한도는 두 배인데 비용은 2.3배가 됐음.
AI로 코드 생산량이 늘고 PM/PD까지 저장소에 기여하기 시작하면 라인은 계속 늘어남. 코드가 늘어난 만큼 비용이 붙는 모델은 이제 우리가 일하는 방식과 맞지 않았음.
특히 가설을 빠르게 검증하려고 가볍게 시작하는 프로젝트가 늘었음. 이런 프로젝트에도 최소한의 안전장치는 필요해서 AI 코드리뷰와 정적분석을 둘 다 붙여야 하는데, 라인 수 기반 과금에서는 새로 시작하는 것 자체가 비용이 됨.
Qodana는 활성 기여자(active contributor) 단위 과금임. 최근 90일 안에 커밋한 사람 수로 계산하고, 우리가 쓰는 Ultimate Plus는 인당 월 $15라 28명 연간 결제로 $5,040(부가세 별도). 같은 시기 SonarQube 2M이 $9,757이었으니 절반 수준이고, 경쟁사 전환 할인 25%까지 받아서 첫해는 60% 가까이 줄었음.
물론 트레이드오프도 있었음. 시크릿 스캐닝, IaC 보안, 런타임 분석 같은 영역은 Qodana가 지원하지 않아서 커버 범위가 좁아지는 걸 감안해야 했음.
정적분석을 CI에 붙이면 어느 도구든 파이프라인 시간이 늘기 때문에, 스캔 단계를 분리하고 무거운 검사는 제외하는 튜닝도 필요했음.
시도 비용이 변동비인 건 LLM만으로 충분함. 나머지는 고정비여야 ROI가 나옴.
